...
class Network:
    ...
    def add_edge(
        self, entity1: int, entity2: int, edge_thickness: int, edge_weight: float
    ):
        edge = (entity1, entity2, edge_thickness, edge_weight)
        self.edges.append(edge)
        self.graph.add_edge(
            entity1, entity2, edge_thickness=edge_thickness, edge_weight=edge_weight
        )
    ...
    def draw(self):
        ...
        )  # Use the 'weight' attribute to influence the layout
        ...
        edge_widths = [
            self.graph[u][v].get("edge_thickness", 1.0) for u, v in self.graph.edges()
        ]
        nx.draw_networkx_edges(self.graph, pos, width=edge_widths, alpha=0.5)

        # Labels for the nodes
        labels = {
            node: data["entity_title"] for node, data in self.graph.nodes(data=True)
        }
